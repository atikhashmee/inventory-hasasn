...
@endifstyle="line-height: 10px;"ka)uantity<t (Taka)</th><tr>}}
                                        <span> <b>Brand</b> {{>pan>tduct_unit_price']* $detail['product_quantity.', ',') rrent_due'];
            $net_outstanding = $net_outstanding;
        @endphp
    dy>
                <tr                <talass="summe                   <td style="line-height: 12px;">
                                 <p>Current Due: {{number_format($current_due, 2, '.', ',')}}<                  es: {{numrmat(($today_sales)2, '.', ',')}}<                  lected: {{numrmat(($tnx_amount)2, '.', ',')}}<           <td>
                                    <strong>Net Outstanding: {{ numrmat($net_outstanding, '.', ',') }}<trong   </tr>
                        </table   </td>        <td width="20%">&nbsp;</td>$discount_amount, 2, '.', ',') }}</td>
                            </tr>
                        </="t-align: cente text-transform: uppercase"> <strong>In Word(Taka):</strong> {{numberToWord(($sub_total - $discount_amount))}}</p/>
        <br />
        <br />
        <br />
        <br />
